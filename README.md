...
> **语言：** 界面跟随系统语言，简体中文与英文都是完整本地化的。想固定成某一种：系统设置 → 通用 → 语言与地区 → 应用程序。
...
首次启动会有一个四步引导，把三区语言、逃生通道、扩展与快捷键权限一次讲清楚（随时可从菜单栏 → **入门指南** 重看）。之后：

1. 菜单栏出现 **⚓ 图标**，进入绿区 app 后屏幕顶部出现一个**呼吸绿点**
...
所有数据保存在 `~/Library/Application Support/Anchor/` 的 SQLite 里（权限 `0600`，仅本人可读），没有账号、没有云、没有任何遥测。**网址只存「域名 + 路径」，`?` 之后的查询串在落盘前就被丢掉**（搜索词、分享令牌不进磁盘）。设置 → 隐私与数据 里可以导出全部数据、一键清除历史、设置保留时长。

唯一的出网行为是**更新检查**（默认关闭）：开启或手动点击时会向 `raw.githubusercontent.com` 发一个 HTTPS 请求，这会让 GitHub 得知你的 IP 与 Anchor 版本。除此之外没有任何网络请求——完整说明见 [PRIVACY.md](PRIVACY.md)。

「为什么不上 Mac App Store？」——沙盒会禁掉前台监控这条命根子，所以走签名公证的直接分发。
